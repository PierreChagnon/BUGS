...
| Colonne | Type | Domaine | Description |
| :-- | :-- | :-- | :-- |
| `acceptability_question_1` | string | `1`…`7`, `NA` *(à venir)* | Réponse 1re question dimension « acceptabilité » — texte : « How helpful did you find your advisor? » (DEC-035). Échelle à **7 points** saisie au **slider** (1 = pas du tout d'accord → 7 = tout à fait d'accord). **Quand `advisor_choice = none`** la question n'est délibérément pas posée : aujourd'hui la colonne est **vide** ; ⏳ **DEC-040 (09/09/26)** acte le passage à un **`"NA"` explicite** (correctif de sérialisation à livrer) — `null`/vide restera réservé à « jamais mesuré » (abandon, lignes antérieures) |
| `acceptability_question_2` | string | `1`…`7`, `NA` *(à venir)* | Réponse 2e question dimension « acceptabilité » — texte : « How satisfied were you with your advisor? » (`746a4406`). Même échelle, même règle sans advisor (DEC-040) |
| `sens_of_agency_question` | string | `1`…`7` | Réponse dimension « sense of agency ». Échelle à 7 points saisie par **7 boutons radio**. (Q-011) |
| `human_likeness_question` | string | `1`…`7` | Réponse dimension « human-likeness » — texte : « How human-like did you find your advisor? » (DEC-035/041). 7 boutons radio. Posée **une seule fois par bloc** (au dernier trial), puis renseignée via PATCH. **La réponse est recopiée sur TOUTES les lignes du bloc** — c'est la **convention actée** (DEC-042, table plate sans jointure), cf. §Limites n°8 pour l'avertissement d'analyse. ⚠️ D-017 : posée aujourd'hui aussi dans les blocs sans advisor, contrairement au protocole validé (DEC-041) — correctif à venir (garde advisor + `"NA"`) |
...
7. **Questions ouvertes impactant le sens** : ✅ **toutes tranchées le 09/09/26** — Q-007 (modèle de pénalités validé, DEC-039), Q-010 (fiabilité probabiliste configurée chercheur, DEC-037), Q-011 (mapping et textes fixés en scène, DEC-035). Voir `decisions.md`.
...
8. **`human_likeness_question` est répliqué sur toutes les lignes du bloc** — comportement **acté** (DEC-042, 09/09/26, cohérent avec la table plate DEC-011). La question n'est posée qu'une fois (dernier trial) mais la réponse est PATCHée sur chaque ligne de 1 à `trial_count` (`ApiClient.cs:150-186`). **Toute moyenne ou tout comptage sur les lignes surpondère la réponse ×`trial_count`.** Dédupliquer par (`participant_id`, `block_index`) avant analyse. Idem, plus largement, pour toutes les variables de niveau bloc recopiées sur chaque trial (choix et conseil distal, paramètres de bloc) — cf. limite 11.
9. 🟠 **Le niveau moteur n'a que la moitié de ses données de résultat** *(reformulée le 09/09/26 — était 🔴 « aucune donnée »)*. Depuis le 08/09/26 (`a919a683`, DEC-030), `motor_advice_visible` et `motor_advice_reliable` sont des **observations réalisées** du trial (à distinguer des paramètres `motor_advice_*_probability`, qui restent de la configuration). En revanche **ni le set de touches actif, ni le set affiché ne figurent dans l'export** : sur un trial libre, le set actif est tiré par le RNG et n'est reconstructible que par re-simulation. **L'adhérence au conseil moteur reste non mesurable.** (D-008 partiellement clos, Q-MOTOR-1)
...
11. **Granularité : 1 ligne = 1 trial**, pas 1 écran — **confirmée par DEC-043 (09/09/26)**. Le tableau de référence client prévoyait une ligne par écran (`screen_type`, `screen_id`). Les informations de niveau bloc — `valley_choice`, `distal_advice_*`, `distal_scene`, `advisor_choice`, tous les paramètres `*_forced*` et map — sont **recopiées à l'identique sur chaque trial du bloc** (convention table plate, cf. DEC-042 pour human-likeness). ⏳ Deux colonnes de repérage `screen_type`/`screen_id` seront ajoutées pour faciliter le filtrage (DEC-043, à livrer). (D-011 clos / Q-ROW-1 close)
12. **Aucune colonne d'adhérence au conseil (`*_match_advice`).** À recalculer post-hoc : distal = `valley_choice` vs `distal_advice_choice` ; chemin proximal = `followed_advisor_path` (déjà fourni) ; cible proximale = `choice_correct` comme proxy **uniquement quand le conseil était fiable** ; moteur = **toujours impossible** — on sait désormais si le conseil était visible et fiable (limite 9, MàJ 09/09/26), mais sans le set actif ni le set affiché, l'adhérence ne se recalcule pas. (D-007)
13. **Deux tirages de présentation sont reproductibles mais non enregistrés** *(reformulée le 09/09/26)*. L'**ordre d'affichage des 3 options d'advisor** est, depuis le 08/09/26 (`88899e9a`, DEC-029), tiré **une fois par bloc, seedé** (salt 7) — plus de remélange à chaque affichage. Le **genre du badge de l'advisor humain** l'est depuis le 29/08/26 (`AdvisorBadgeUtility`, commit `48b05b25`, salt 6). **Mais ni l'un ni l'autre n'est exporté dans `trial_responses`** : `advisor_display_order` et `advisor_display_is_male` vivent dans l'état de session, jamais recopiés dans la ligne. Un effet de position ou de genre ne se vérifie donc que par re-simulation hors ligne depuis la seed de bloc — pas depuis l'export. ⚠️ Le pilotage a acté le 09/09/26 que l'export du tirage réalisé de l'ordre est **requis** (MàJ DEC-029). Le reste du gameplay est reproductible depuis `trial_seed`. (D-012, Q-RANDOM-1)
