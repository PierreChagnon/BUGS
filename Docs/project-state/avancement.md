...
| WelcomeScene | ✅ Créée |
| AdvisorChoiceScene + AdvisorChoiceUI + AdvisorOptionButton | ✅ **E3 réglé le 09/09/26** (`13bd070f`) — texte de dev supprimé (`_titleText`/`_subtitleText` retirés). Ordre des options seedé par bloc depuis DEC-029 |
...
| ~~QuestionnaireScene + QuestionnaireUI~~ (DEC-008 annulée) | ✅ **Supprimée le 09/09/26** (DEC-038, commits PC `b6fb5258`/`13bd070f`, vérifié) — scène, `QuestionnaireUI`, `QuestionConfig` et `GamePhase.Questionnaire` retirés. E1/N1-D/D-010 clos |
...
| ~~**E3**~~ | AdvisorChoiceScene | ✅ **Réglé le 09/09/26** (`13bd070f`) — le texte de dev « Ce choix est enregistre mais n'affecte pas encore la generation de la map » et les champs `_titleText`/`_subtitleText` sont supprimés. |
...
| **QuestionnaireScene ne collecte rien** (E1) | Écran mort dans le flow | ✅ **Levé le 09/09/26** (DEC-038) — scène supprimée (`b6fb5258`/`13bd070f`, vérifié) |
...
3. ~~Nettoyer les textes placeholders~~ ✅ **Fait le 09/09** — E3 supprimé par PC (`13bd070f`) et F6 clos (DEC-035 + `746a4406`). **Plus aucun placeholder visible participant.**
