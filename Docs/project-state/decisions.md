...
- **Mise à jour 2026-09-09 (soir) — retrait livré et vérifié :** commits PC `b6fb5258`/`13bd070f` (−3653 lignes) : scène supprimée + hors du build, `QuestionnaireUI`, `QuestionConfig`, `GamePhase.Questionnaire` et `OnQuestionnaireComplete` nettoyés. **E1/N1-D clos, D-010 clos.** Le même commit supprime le texte de dev d'`AdvisorChoiceUI` (`_titleText`/`_subtitleText`) — **E3 clos** par la même occasion.
- **Statut :** ACTIF
...
- **MàJ 2026-09-09 :** point (2) amendé par DEC-045 — le garde runtime `is_tutorial` est retiré, la consigne dashboard « tutoriel sans explanations » est levée ; le statu quo du report est conservé.
- **Statut :** ACTIF *(pt 2 amendé par DEC-045)*

### DEC-045 — Explanations affichées en bloc tutoriel (amende DEC-044 pt 2, supprime R4/TR4 de la spec Explanations)
- **Date :** 2026-09-09
- **Tag :** [FONC] / [TECH]
- **Décision :** Les explanations (distal, proximal, motor) **peuvent s'afficher en bloc tutoriel**, exactement comme sur un bloc normal : le garde `is_tutorial` de `ExplanationResolver.Resolve` est **retiré**. Seule la configuration du bloc (`display_mode`, `display_probability`, corpus) et les préconditions R1 (advisor choisi + advice visible) décident de l'affichage. Le Communication Report continue de s'afficher en tutoriel (statu quo DEC-044 pt 2, inchangé).
- **Raison :** La règle « pas d'explanation en tutoriel » (R4/TR4 de `explanations-short-long/spec-fonc.md`) n'avait jamais été arbitrée par les chercheurs : elle avait été posée par l'analyse fonctionnelle (mai 2026) par simple cohérence avec « pas d'envoi API en tuto » (DEC-014), puis codée telle quelle le 02/06 (`40861eb8`). Or le tutoriel sert à familiariser le participant avec **toutes** les mécaniques, explanations comprises — les découvrir au premier bloc réel biaise ce bloc. De plus, ce garde runtime était la seule source d'incohérence entre le Communication Report (qui lit la config) et ce que le participant voyait réellement : en le retirant, report et explanations dérivent de la **même** config, et la consigne opérationnelle « tutoriel sans explanations » de DEC-044 devient sans objet.
- **Impact :** Code : `ExplanationResolver.Resolve` ne teste plus `block.is_tutorial` (1 ligne). Aucun impact API : les trials tutoriels ne sont toujours pas envoyés (`TrialManager`, DEC-014) — les colonnes `*_explanation_*` restent donc absentes de la base pour le tuto, sans règle dédiée. Spec fonc Explanations : R4 et TR4 réécrites (le tutoriel suit les règles générales), ligne hors-scope « Tutoriel » retirée, question ouverte §2.4 close. DEC-044 pt 2 : le statu quo du report est conservé, la **consigne dashboard est levée** — le chercheur peut configurer des explanations sur le bloc tutoriel s'il le souhaite. Q-EXP-11 : réponse complétée. TDD §5.3 mis à jour. Prérequis opérationnel : le bloc tutoriel doit porter une section `explanations` avec des textes pour qu'une explanation apparaisse (comme tout bloc).
- **Statut :** ACTIF

### DEC-046 — Explanations : le texte proposé remplace l'id, et la config du bloc est échoée dans `trial_responses` (amende DEC-018 « enum + id »)
- **Date :** 2026-09-14
- **Tag :** [FONC] / [TECH]
- **Décision :** Les 3 colonnes `*_advice_explanation_text_id` sont remplacées par `*_advice_explanation_text` : le **texte du corpus proposé sur le trial**, copié tel quel par Unity. Le texte est renseigné dès que le mode réalisé est `forced` ou `opt-in` — y compris en opt-in jamais ouvert (`clicked = false`, `display_duration_ms = 0`) — et `null` uniquement quand rien n'a été proposé (`display_mode = none`). Le champ `id` disparaît du corpus (`ExplanationText`, schéma backend, base). En parallèle, la **config explanations du bloc est échoée** dans chaque ligne, au même titre que `proximal_forced_probability` : `*_configured_display_mode`, `*_display_mode_forced_probability`, `*_configured_content_variant`, `*_content_variant_long_probability`, et `*_display_probability` sur proximal/motor (14 colonnes). Le tirage `display_probability` obtient sa colonne réalisée `*_advice_explanation_visible` (proximal/motor, `true`/`false`, `null` quand l'advice était caché et qu'aucun tirage n'a eu lieu), sur le patron de `motor_advice_visible` — le chercheur n'a plus à déduire la cause d'un `none` par élimination.
- **Raison :** DEC-018 (Q-EXP-9, option C « enum + id », « Maximum traceability! ») visait à **retrouver le contenu exact** affiché. L'id ne le permettait pas : personne ne l'attribuait (le dashboard le forçait à `''`), aucune table ne le reliait à un texte, et le texte n'était archivé nulle part — le seul moyen de le retrouver était de relire la config du bloc, faussée dès qu'un chercheur la modifiait après passation. Archiver le texte remplit l'intention de DEC-018 sans dépendre d'un id. L'écho de la config répond au constat que les paramètres de session des explanations (`display_probability`, probabilités des valeurs mixtes) ne remontaient pas, et qu'un `none` réalisé ne se distinguait pas d'un tirage négatif (divergence D-013).
- **Impact :** Unity : `ExplanationText.id` et `ExplanationRuntimeState.text_id` supprimés ; `TrialResponseRow` : `*_text_id` → `*_text` + 14 champs d'écho + 2 `*_visible` (`ExplanationRuntimeState.visible`, posé par `FlowController.RollExplanationProbability`, désormais `bool?`) ; `FlowSerializationUtility.ApplyExplanationState(row, level, config, state)` ; `FlowController.ApplyCurrentExplanationStatesToRow` passe la config du niveau. Backend (`backend-bugs`) : migration `20260914120000_replace_explanation_text_id_with_text_and_config.sql`, `trialResponseSchema`, corpus `{ text }` sans `id`, export CSV 95 → 111 colonnes. Docs : spec fonc Explanations §2 et §4, TDD §5.1/§5.3, `matrice-tracabilite.md`, `dictionnaire-donnees.md`, `config-session-test.md`, Q-EXP-9. Déploiement : backend, migration et build Unity ensemble — les anciennes builds envoient `*_text_id`, refusé par le schéma strict.
- **Statut :** ACTIF

## Index par tag

- **[SCOPE]** : DEC-004, DEC-005, DEC-008 *(annulée)*, DEC-024, DEC-031, DEC-033, DEC-034, DEC-036, DEC-038
- **[FONC]** : DEC-001, DEC-003 *(annulée)*, DEC-006 *(résolu)*, DEC-010, DEC-012, DEC-017, DEC-018 *(colonne id amendée par DEC-046)*, DEC-019, DEC-023, DEC-025, DEC-028, DEC-029, DEC-031, DEC-032, DEC-033, DEC-034, DEC-035, DEC-037, DEC-039, DEC-040, DEC-041, DEC-042, DEC-043, DEC-044 *(pt 2 amendée par DEC-045)*, DEC-045, DEC-046
- **[TECH]** : DEC-002, DEC-007, DEC-009, DEC-011, DEC-013 *(amendée par DEC-042)*, DEC-014, DEC-015, DEC-016, DEC-020, DEC-021, DEC-022, DEC-026, DEC-027, DEC-029, DEC-030, DEC-036, DEC-040, DEC-042, DEC-043, DEC-045, DEC-046
